...
- **Notes:** **IN REVIEW — needs independent review by core-engineer** (qa-gated + qa-tuning's own task). **Outcome: precision 0.60 → 0.75 on the seeded corpus** (the achievable ceiling; the lone remaining false fire is `ff-false-wrong-category`, a detector mis-naming no orchestrator/threshold lever can fix).
