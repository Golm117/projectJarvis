...
- **Claimed:** 2026-06-16T22:00:00Z
- **Completed:** 2026-06-16T23:30:00Z
- **Depends on:** docs/ml/interjection-prior-art.md (research complete)
- **Description:** Rework `QwenWallBackend` (`src/jarvis/ml/wall.py`) and the cheap pre-filter (`_has_wall_signal` in `attention_layer.py`) based on the research in `docs/ml/interjection-prior-art.md`. Root cause: the model fires on "what's 4 times 7?" but not "what's the square root of 81?" because (1) the pre-filter silently skips non-`?`-terminated gap phrasings, and (2) the near-binary ~0.95 confidence from the current prompt makes the `SummonController` 0.70 floor inert (T-503 finding). Four concrete changes: (a) **graded 1–5 rating** replacing near-binary confidence — Inner-Thoughts style, mapped into `WallVerdict.confidence`; derive `is_wall` from rating≥3. (b) **Information-Gap structured CoT** before scoring: the model reasons about unanswered-ness, factual-answerability, directedness (group vs Jarvis/self/rhetorical), and offer-helpfulness before scoring. (c) **Few-shot exemplars** from our real failure cases anchoring consistency. (d) **Widened `_has_wall_signal` pre-filter** to admit "I wonder…", gap phrasings, and wh- forms that don't end in `?`. Model-free tests pin the classification; live run on the failure cases measures actual consistency. qa-GATED (WallDetector behavior change).
- **Acceptance:**
  - `QwenWallBackend` prompt reworked: graded 1–5 rating → calibrated confidence mapping; Information-Gap CoT; few-shot exemplars covering √81 miss, "what do you need?" FP, self-musing FP, declarative gap medium.
  - `_has_wall_signal` widened in `attention_layer.py`: "I wonder…" + additional gap phrasings admitted; non-gaps still rejected; cost note documented.
  - `WallVerdict` shape preserved (frozen); `confidence` now carries the graded signal.
  - Model-free tests: rating→confidence mapping; stubbed `generate` classifies √81 → high factual_gap; "what do you need?" → none; self-musing → none; widened pre-filter passes previously-skipped gaps; robust JSON parse with `reasoning`+`rating` fields; graceful fallback.
  - Existing model-free test suite stays green (480 baseline); ruff clean.
  - Live results on √81 / 4×7 / "what do you need?" reported honestly.
...
- **Notes:** **qa-GATED. Route to qa-tuning before marking done.**
  ...
  **qa-tuning verdict (2026-06-16): APPROVED → done.** Full review record in `docs/qa/threshold-tuning.md` §7.

  - **Implementation soundness — confirmed.** Diff touches only `src/jarvis/ml/wall.py` (prompt + parse) and `src/jarvis/attention_layer.py` (the `_RE_WALL_SIGNAL` pre-filter + docstring). The frozen `WallVerdict` shape is unchanged; `SummonController`, `TurnTakingGate`, `WallDetector`, `types.py` are **byte-for-byte unchanged** since before T-508 (`git diff b8cdee5 HEAD` empty on all four). Rating→confidence mapping is sane and floor-aware (1/2/3 below 0.70, 4/5 above). Graceful `none()` fallback intact across the full parse-failure matrix. `is_wall` correctly derived from `rating>=3`, NONE-iff-¬is_wall invariant enforced. Pre-filter widening is reasonable + cost-documented (~392 ms detector cost, ~1,065 ms joint margin; precision-first model handles residual self-musing FP risk via exemplar). 100 model-free tests assert external behavior (returned `WallVerdict` fields + message structure + the public `rating_to_confidence` mapping) over stubbed `generate` — no real model in unit tests. Suite **523 green**, ruff lint+format clean.
  - **Precision finding.** Eval scores **labels + config**; the fixtures carry `observed_confidence` from the OLD near-binary backend, so the committed corpus still scores **0.75** (T-503 ceiling; lone false fire = `ff-false-wrong-category`). T-508 changes how the backend *produces* confidence, not the labeled fixture values. Modeled-graded sweep (§7.3) shows the floor — **inert** under the old backend — now does **real work** between rating tiers (the §1 "inert" verdict was backend-version-specific; T-508 lifts it).
  - **Floor recommendation (NOT finalized — left at 0.70 in code; human sign-off requested).** **Keep `interjection_confidence_floor = 0.70`**, now for a *sound* reason rather than inertness: under graded confidence it cleanly admits rating-4/5 (0.80/0.95) and suppresses rating-1/2/3 (0.05/0.30/0.65) — the precision-first boundary we want. The only judgement call is whether rating-3 (0.65) wh-form/declarative gaps should speak; lowering to 0.65 is a recall move on a precision-first, FP-limited corpus (not recommended). Raising to 0.75 is cosmetic (nothing lands in (0.70,0.80)). Sweep tables in §7.2–7.4.
...
- **Completed:** 2026-06-16T23:59:00Z
...
- **Progress:**
  ...
  - 2026-06-16 — **qa-tuning GATE: APPROVED → done.** Real-path `detect_wall` validation (multi-line rolling-window, 4 runs each) + 2 live `--capture` runs on 7B. Full results in `docs/qa/threshold-tuning.md` §7.
- **Notes:** **DONE — qa-tuning APPROVED (2026-06-16).** SOUNDNESS: frozen `WallVerdict`/`SummonController`/`TurnTakingGate`/`WallDetector` byte-for-byte unchanged since `b8cdee5` (`git diff` empty on all four); 7B switch is a clean one-line `DEFAULT_MODEL_PATH` change (3B still selectable by injection; one shared `QwenModel` feeds both backends); graded 1–5→confidence infra intact. Suite **529 green** (527 + 2 new Scenario-D tests), ruff check + format clean. **REAL-PATH `detect_wall` (multi-line, the core of this gate):** T-508 regression IS fixed — direct unanswered questions fire in multi-line context: "4×7?" 4/4 fire (rating 5 @ 0.95), "conference date?" 4/4 fire. WDYN-post-summon, self-musing, plain statements all 0/4 (rating 1) — correct silence. **One miss found + characterized:** "√81?" in a *dense* 4-line homework window misses 5/5 (rating 1) because the 7B *confabulates* "but Alice answered it" (she didn't) — context-sensitive (√81 single-/two-line both fire rating 5; same density fires 4×7), and **does NOT reproduce live** (two live `--capture` runs fired √81 → "That's 9." @ 0.95). A miss is *silence on a gap* = **recall** cost, never precision — precision-first is the metric, so NOT a blocker. **WDYN (Scenario D) resolved:** non-issue for production — suppressed by BOTH the 7B (rates it none) AND the T-503 6 s post-engagement cooldown (load-bearing, model-independent); pinned in `test_scenario_d_wdyn_within_cooldown_is_suppressed`. **PRECISION:** committed-corpus eval **0.75, not regressed**. **FLOOR:** keep 0.70 — sound, separates the rating tiers the 7B emits (1/4/5 observed). **HUMAN/ROUTING ITEMS:** (1) **local-ml-engineer** — v1 prompt lever for the √81 dense-context confabulation (exemplar: "do not assume a question was answered unless the answer appears verbatim"); recall, not a v0 blocker. (2) **core-engineer** — optional permanent engaged-state `detect_wall` suppression for the WDYN >6 s residual (qa-gated; not built by qa); residual documented + pinned. (3) **T-504** — 7B is now the always-on default; thermal/battery cost of 7B (vs 3B) sustained load is unmeasured (T-504 deferred to real-world use). Full qa write-up: `docs/qa/threshold-tuning.md` §7.

### T-510 — Confabulated-answer guard in wall detector (√81 dense-context miss)
- **Status:** done
- **Priority:** P2
- **Role:** local-ml-engineer
- **Owner:** local-ml-engineer
- **Phase:** 5
- **Created:** 2026-06-16T23:59:00Z
- **Claimed:** 2026-06-16T23:59:00Z
- **Depends on:** T-509 (done — surfaced this as v1 prompt-lever routing item #1)
- **Description:** Implements the v1 prompt lever T-509's qa gate routed to local-ml-engineer. The 7B wall detector (`src/jarvis/ml/wall.py`) has a context-sensitive RECALL miss (T-509 gate, `docs/qa/threshold-tuning.md` §7.2): in a *dense* multi-line transcript it confabulates that an interlocutor already answered a trivially-knowable unanswered question (e.g. "What's the square root of 81?") and rates it 1/none. Captured `reasoning`: "Bob asked a direct arithmetic question, but Alice answered it. No gap." — Alice did NOT answer it. Precision-preserving prompt-only guard so the model does not assume a question was answered unless an answer appears VERBATIM in the transcript. qa-GATED (WallDetector behavior change).
- **Acceptance:**
  - Prompt-only change to `wall.py` (`_REASONING_INSTRUCTION` step 4 verbatim-answer rule + new Example 8); `WallVerdict`/`SummonController`/`TurnTakingGate`/`WallDetector` byte-for-byte unchanged.
  - The dense 4-line "geometry homework … What's the square root of 81?" transcript (question followed by an off-topic line) now FIRES on the real `detect_wall(transcript, summary)` path.
  - No precision regression: genuinely-answered control, self-musing, plain statements STILL stay silent.
  - `~/.local/bin/uv run pytest` GREEN + ruff clean.
  - qa-tuning mandatory gate before merge.
  - Do NOT mark `done` (awaiting commit/human sign-off).
- **Progress:**
  - 2026-06-16 — Reproduced the documented confabulation DETERMINISTICALLY (4/4) on the real `detect_wall` path: question followed by a subject-changing line → `is_wall=False @ 0.05`, reasoning "Bob asked a direct arithmetic question, but Alice answered it before the transcript ended."
  - 2026-06-16 — Fix applied: `_REASONING_INSTRUCTION` step 4 now requires a VERBATIM answer (a later line that changes subject/acknowledges/stays silent is NOT an answer); new Example 8 models the dense-context trap (√81 asked, next line changes subject, rating 5).
  - 2026-06-16 — Validated (5 runs each): confab-trap now 5/5 FIRE; 4×7/conference-date/single-line √81 5/5 FIRE; genuinely-answered control 0/5 SILENT (guard does NOT over-fire); self-musing/plain-statement 0/5 SILENT. WDYN proven PRE-EXISTING (5/5 fire BOTH before and after via git-stash A/B) — out of scope, covered by the T-503 cooldown.
  - 2026-06-16 — **qa-tuning GATE: APPROVED.** Independent real-path re-validation (5 runs each): confab-trap 5/5 fire, answered-control + self-musing + plain-statement 0/5; corpus precision 0.75 unchanged; 529 pytest green; ruff clean; live gate test passes. Full write-up `docs/qa/threshold-tuning.md` §8.

### T-511 — Fix answered-question over-fire (4×7 precision bug) — CANCELLED (v1 limit)
- **Status:** cancelled
- **Priority:** P2
- **Role:** local-ml-engineer
- **Phase:** 5
- **Created:** 2026-06-16
- **Depends on:** T-510
- **Description:** Pre-existing precision bug — a factual question asked AND answered in the same rolling window over-fires `unanswered_question @ 0.95` (the 7B fails to read the answer line and confabulates "nobody answered").
- **Notes:** **CANCELLED 2026-06-17 — NOT promptable on the 7B.** Two prompt iterations each fixed only the exact exemplar strings and broke on the next phrasing. Decisive real-path probe (5 runs each): `What's 4 times 7?` / `It's 28.` → 0/5 (exemplar string) BUT `What is 4 times 7?` / `It's 28.` → 5/5, and √81-answered → 5/5. The 7B pattern-matches the exact question string, not answer-reasoning → infinite whack-a-mole; r2 even regressed the √81-answered control. Human decision (2026-06-17): **stop, revert to the T-510 baseline (`9ac28c5`), document as a known v1 limit** (fixable only by fine-tuning, per `docs/ml/wall-detector-v1-limits.md` + `interjection-prior-art.md`). Discarded iteration commits (reflog-recoverable): `4d72de9 8a4af08 4f76769 a2ce5e4 c8d2e42`. Narrow edge case (asked+answered+re-detected in one window); NOT a v0 blocker — live runs never hit it.
  - 2026-06-16 — **qa-tuning SECOND independent gate (this session): APPROVE.** Re-ran real 7B `detect_wall`, my own probes + 2 added stress probes, 5 runs each. Confab-trap A 5/5 fire, variant A2 5/5 fire, √81-answered control B 0/5, self-musing/plan/WDYN 0/5, 4×7-unanswered F 5/5. **Caveat (honest):** a *genuinely-answered* 4×7 ("Bob: That's 28.") over-fires 5/5 — the model's reasoning reads "nobody answered". **git-stash A/B proves this is PRE-EXISTING** (identical 5/5 over-fire on the pre-T-510 prompt `57e409e^`; T-510 did not cause/worsen it) and the structurally-analogous √81-answered case is correctly silent both before & after, so the verbatim rule does NOT over-generalise → **T-510 is precision-neutral + recall-positive.** Frozen-module diff since b8cdee5 = 0 lines. Corpus precision 0.75 (ff-false-wrong-category lone false fire); 529 pytest; ruff clean; live gate + T-503 (14) green. Write-up §8.6. Pre-existing 4×7-answered over-fire spawned as a separate tracked task.
- **Completed:** 2026-06-16 — qa-tuning gate APPROVED (independent real-path 7B validation, §8.6). T-510 recovers the √81 confab miss at zero precision cost. Pre-existing 4×7-answered detector over-fire is out of scope (separate task), not a T-510 blocker.
- **Notes:** Closes T-509 routing item #1 (local-ml-engineer v1 prompt lever). Recall-only lever — precision (the success metric) unaffected. qa-gate done; orchestrator pushes on approval.
